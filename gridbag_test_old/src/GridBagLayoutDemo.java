...
public class GridBagLayoutDemo {
	...
	private String linea, dir, mowpos, _newmowpos;
	...
	public int matr2button(String _matr) {
		...
		try {
			...
			// if button is in the first row, select the column, otherwise
			// calculate: 5. button - column
	...
	public void setinitialmower() {
		...
		try {
			ImageIcon image_working_mower = new ImageIcon(
					"src/mower_working.png");
	 ...
	 */
	public void setmowerpos(final String _newmowpos) {
		
		int delay = 1000;
		
		
		ActionListener cortarhierba = new ActionListener(){
			...
			myButton[matr2button(_newmowpos)].setActionCommand(mowpos + "r");
		...
		new Timer(delay,cortarhierba).start();
	...
	public void setfields(Container pane) {
		...
		ActionListener myListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// check for type of button: button contains a number,
				// radiobutton doesnt
				if (s.matches(".*\\d.*")) {
					...
					System.out.println(button2matr(localbutton)
							+ "from obstacles");
					if (obstacletype == 1) {
				...
				// when radiobutton was selected, set either tree or cat to put
				// as a new image
			...
			for (setrow(1); getrow() <= dim; setrow(getrow() + 1)) {
				for (setcolumn(1); getcolumn() <= dim; setcolumn(getcolumn() + 1)) {
					...
					BufferedImage buttonIcon = ImageIO.read(new File(
							"src/hierba_small.png"));

					myButton[index] = new JButton(new ImageIcon(buttonIcon));

					myButton[index].setActionCommand("r" + getrow() + ",c"
							+ getcolumn());
